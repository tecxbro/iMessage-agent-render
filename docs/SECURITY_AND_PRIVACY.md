...
The setup dashboard is a separate operator boundary from iMessage sender authorization. `DASHBOARD_SETUP_SECRET` is generated by Render for production and is entered only in the server-submitted **Deployment setup code** form. The owner phone number remains a Render Blueprint input until Prompt 2.
