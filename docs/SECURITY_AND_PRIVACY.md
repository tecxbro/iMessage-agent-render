...
- Dashboard setup secret, operator session identifiers, and CSRF tokens.
...
| Render-generated dashboard setup secret | trusted operator credential; never expose to the browser except as submitted login input |
...
The setup dashboard is a separate operator boundary from iMessage sender authorization. `DASHBOARD_SETUP_SECRET` is generated by Render for production and is entered only in the server-submitted **Deployment setup code** form. Only after that boundary is active may the operator submit the owner phone. The write additionally requires the live session's CSRF token and same-origin request headers.

The server:

1. compares submitted setup-secret bytes in constant time;
2. rate-limits failed authentication attempts;
3. creates opaque session IDs and CSRF tokens with cryptographically secure random bytes;
4. stores at most eight active sessions in bounded server memory;
5. expires each session after eight hours and cleans up expired records; and
6. treats logout and repeated revocation as idempotent.

A service restart invalidates all operator sessions. The cookie contains only the session ID, never the setup secret or CSRF token, and uses `HttpOnly`, `SameSite=Strict`, `Path=/`, `Secure` in production, and no `Domain` attribute. Authentication state must not use local storage, session storage, URL query parameters, URL fragments, or a client-readable cookie. The setup secret must not be rendered into HTML or JavaScript.
...
| `/agent/dashboard` | Deployment setup-code login only; operational dashboard requires a session | None for `GET` |
...
| Secret | Storage |
|---|---|
| Dashboard setup secret | Generated Render Web Service environment; high-entropy `.env` value for local development |
| Operator sessions | Bounded in-memory server store; never the setup secret |
...
No submitted setup secret, session ID, CSRF token, device code, verification URL, raw message, prompt, full command output, handle, auth token, or environment dump by default.
...
| Setup-secret guessing | constant-time comparison and failed-attempt rate limit |
...
- Logs contain no setup secrets, session IDs, CSRF tokens, device codes, or verification URLs.
