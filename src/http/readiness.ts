...
const SAFE_ACTIONS: Readonly<Partial<Record<string, string>>> = {
  ...
  OWNER_IDENTITY_NOT_CONFIGURED:
    "Authenticate to the agent dashboard and save the one owner phone number before starting Photon.",
  OWNER_IDENTITY_MIGRATION_REQUIRED:
    "Resolve the legacy owner configuration to one E.164 phone number, then save it in the authenticated dashboard.",
