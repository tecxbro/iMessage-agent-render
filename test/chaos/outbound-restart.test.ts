...
describe("outbound partial-send recovery", () => {
  it("retries the same GUID after acknowledgement before cursor persistence", async () => {
    ...
    const handler = createOutboundSendHandler({
      ...
      afterAcknowledgement: (acknowledgedPart) => {
        if (acknowledgedPart.position === 1 && crashOnce) {
          crashOnce = false;
          throw new Error("simulated process death after provider acknowledgement");
        }
      },
    });
